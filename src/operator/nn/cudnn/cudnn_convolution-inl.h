 ...
 */
template <typename DType> class CuDNNConvolutionOp : public Operator {
public:
  explicit CuDNNConvolutionOp(const ConvolutionParam &param,
    ...
    // the global policy.
    cudnn_tensor_core_ =
        DataType<DType>::kFlag == kFloat16 && GetEnvAllowTensorCore();
...
#else
    CHECK(param_.layout.value() == kNCW || param_.layout.value() == kNCHW ||
    ...
    Engine::Get()->PushAsync(
        [=](RunContext rctx, Engine::CallbackOnComplete on_complete) {
          ...
          CUDNN_CALL(cudnnGetConvolutionBackwardDataAlgorithm(
              s->dnn_handle_, filter_desc_, out_desc_, back_conv_desc_,
              in_desc_, CUDNN_CONVOLUTION_BWD_DATA_SPECIFY_WORKSPACE_LIMIT,
              workspace_byte, &fastest_bwd_data_algo));
  ...
  virtual void Forward(const OpContext &ctx, const std::vector<TBlob> &in_data,
    ...
    Tensor<gpu, 1, DType> workspace =
        AllocateTempWorkspace(ctx, forward_workspace_byte_);
    size_t workspace_size = TensorSizeBytes(workspace);

    // I/O's should have 2 more dims than the kernel dim
    DType *data_ptr =
        GetNdPtr(in_data[conv::kData], param_.kernel.ndim() + 2, s);
    DType *wmat_ptr =
        GetNdPtr(in_data[conv::kWeight], param_.kernel.ndim() + 2, s);
    DType *out_ptr =
        GetNdPtr(out_data[conv::kOut], param_.kernel.ndim() + 2, s);
    ...
    for (uint32_t g = 0; g < param_.num_group; ++g) {
      ...
      CUDNN_CALL(cudnnConvolutionForward(
          ...
          forward_algo_.AlgoNumber(), workspace.dptr_, workspace_size,
      ...
      if (!param_.no_bias) {
        ...
        CUDNN_CALL(cudnnAddTensor(s->dnn_handle_, &alpha, bias_desc_,
                                  bias.dptr_ + bias_offset_ * g, &beta_add,
                                  out_desc_, out_ptr + out_offset_ * g));
#endif
#if CUDNN_MAJOR == 3
        CUDNN_CALL(cudnnAddTensor(s->dnn_handle_, CUDNN_ADD_SAME_C, &alpha,
                                  bias_desc_, bias.dptr_ + bias_offset_ * g,
                                  &beta_add, out_desc_,
                                  out_ptr + out_offset_ * g));
#endif
      }
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    // I/O's should have 2 more dims than the kernel dim
    DType *grad_ptr =
        GetNdPtr(out_grad[conv::kOut], param_.kernel.ndim() + 2, s);
    DType *wmat_ptr =
        GetNdPtr(in_data[conv::kWeight], param_.kernel.ndim() + 2, s);
    DType *gwmat_ptr =
        GetNdPtr(in_grad[conv::kWeight], param_.kernel.ndim() + 2, s);
    DType *data_ptr =
        GetNdPtr(in_data[conv::kData], param_.kernel.ndim() + 2, s);
    DType *gdata_ptr =
        GetNdPtr(in_grad[conv::kData], param_.kernel.ndim() + 2, s);

    Tensor<gpu, 1, DType> workspace =
        AllocateTempWorkspace(ctx, backward_workspace_byte_);
    size_t workspace_size = TensorSizeBytes(workspace);
    for (uint32_t g = 0; g < param_.num_group; ++g) {
      typename DataType<DType>::ScaleType alpha = 1.0f;
      typename DataType<DType>::ScaleType beta = 0.0f;
      ...
      if (!param_.no_bias && (req[conv::kBias] != kNullOp)) {
        ...
        CUDNN_CALL(cudnnConvolutionBackwardBias(
            s->dnn_handle_, &alpha, out_desc_, grad_ptr + out_offset_ * g,
      ...
      if (req[conv::kWeight] != kNullOp) {
        ...
        CUDNN_CALL(cudnnConvolutionBackwardFilter_v3(
            s->dnn_handle_, &alpha, in_desc_, data_ptr + data_offset_ * g,
            out_desc_, grad_ptr + out_offset_ * g, back_conv_desc_w_,
            back_algo_w_.AlgoNumber(), workspace.dptr_, workspace_size,
            req[conv::kWeight] == kAddTo ? &beta_add : &beta, filter_desc_,
      ...
      if (req[conv::kData] != kNullOp) {
        ...
        CUDNN_CALL(cudnnConvolutionBackwardData_v3(
            s->dnn_handle_, &alpha, filter_desc_, wmat_ptr + weight_offset_ * g,
            out_desc_, grad_ptr + out_offset_ * g, back_conv_desc_,
            back_algo_.AlgoNumber(), workspace.dptr_, workspace_size,
  ...
  static bool Supports(ConvolutionParam param, int forward_compute_type,
                       int backward_compute_type, const Context &ctx) {
    using namespace mshadow;
...
private:
  ...
  void InitDescriptors(const Context &ctx, const std::vector<TShape> &in_shape,
                       const std::vector<TShape> &out_shape,
...
// convolution calculation.  Supply this method signature for cuDNN versions <
// 6.
#define cudnnSetConvolution2dDescriptor(cdesc, p0, p1, s0, s1, d0, d1, m, ct)  \
  cudnnSetConvolution2dDescriptor(cdesc, p0, p1, s0, s1, d0, d1, m)
    ...
    if (param_.kernel.ndim() == 1 || param_.kernel.ndim() == 2) {
      // 1d or 2d conv
      auto pad =
          param_.kernel.ndim() == 2 ? param_.pad : TShape({0, param_.pad[0]});
      auto stride = param_.kernel.ndim() == 2 ? param_.stride
                                              : TShape({1, param_.stride[0]});
      auto dilate = param_.kernel.ndim() == 2 ? param_.dilate
                                              : TShape({1, param_.dilate[0]});
      CUDNN_CALL(cudnnSetConvolution2dDescriptor(
          forward_conv_desc_, pad[0], pad[1], stride[0], stride[1], dilate[0],
          dilate[1], CUDNN_CROSS_CORRELATION, cudnn_forward_compute_type));
      CUDNN_CALL(cudnnSetConvolution2dDescriptor(
          back_conv_desc_, pad[0], pad[1], stride[0], stride[1], dilate[0],
          dilate[1], CUDNN_CROSS_CORRELATION, cudnn_backward_compute_type));
      CUDNN_CALL(cudnnSetConvolution2dDescriptor(
          back_conv_desc_w_, pad[0], pad[1], stride[0], stride[1], dilate[0],
          dilate[1], CUDNN_CROSS_CORRELATION, cudnn_backward_compute_type));
#if CUDNN_MAJOR < 5
...
#define cudnnSetFilter4dDescriptor(fdesc, dt, f, w0, w1, w2, w3)               \
  cudnnSetFilter4dDescriptor(fdesc, dt, w0, w1, w2, w3)
      CHECK_EQ(format_, CUDNN_TENSOR_NCHW)
          << "CuDNN V4 and earlier only supports NCHW layout";
#endif
      if (param_.kernel.ndim() == 2) {
        wshape = ConvertLayout(wshape.get<4>(), param_.layout.value(), kNCHW);
        dstride =
            ConvertLayout(Strides<4>(dshape), param_.layout.value(), kNCHW);
        dshape = ConvertLayout(dshape.get<4>(), param_.layout.value(), kNCHW);
        ostride =
            ConvertLayout(Strides<4>(oshape), param_.layout.value(), kNCHW);
        ...
        dstride =
            ConvertLayout(Strides<3>(dshape), param_.layout.value(), kNCW);
...
// 3d conv
      ...
      CUDNN_CALL(cudnnSetConvolutionNdDescriptor(
          back_conv_desc_, 3, param_pad_.data(), param_stride_.data(),
          param_dilate_.data(), CUDNN_CROSS_CORRELATION,
          cudnn_backward_compute_type));

      CUDNN_CALL(cudnnSetConvolutionNdDescriptor(
          back_conv_desc_w_, 3, param_pad_.data(), param_stride_.data(),
          param_dilate_.data(), CUDNN_CROSS_CORRELATION,
          cudnn_backward_compute_type));

      dstride =
          ConvertLayout(Strides<5>(dshape), param_.layout.value(), kNCDHW);
      dshape = ConvertLayout(dshape.get<5>(), param_.layout.value(), kNCDHW);
      ostride =
          ConvertLayout(Strides<5>(oshape), param_.layout.value(), kNCDHW);
...
// Set "allow tensor core" flag in convolution descriptors, if available.
    ...
    cudnnMathType_t math_type =
        cudnn_tensor_core_ ? CUDNN_TENSOR_OP_MATH : CUDNN_DEFAULT_MATH;
    ...
    if (!param_.no_bias) {
      ...
      CUDNN_CALL(cudnnSetTensorNdDescriptor(bias_desc_, dtype_,
  ...
  void SelectAlgo(const Context &ctx, const std::vector<TShape> &in_shape,
                  ...
                  cudnnDataType_t cudnn_forward_compute_type,
                  cudnnDataType_t cudnn_backward_compute_type) {
    if (!CuDNNConvAlgoReg::Get()->Find(
            param_, in_shape, out_shape, dtype_, cudnn_forward_compute_type,
            cudnn_backward_compute_type, SMArch(ctx.dev_id), &forward_algo_,
      ...
      Engine::Get()->PushAsync(
          [=](RunContext rctx, Engine::CallbackOnComplete on_complete) {
            ...
            size_t workspace_byte =
                static_cast<size_t>(param_.workspace * sizeof(DType));
#if CUDNN_MAJOR >= 7
            // Starting with cuDNNv7, the algo number returned by *Get*() is not
            // the entire
            // story: the notion of whether the algo ran in Tensor Core mode is
            // not known.
            // Since we want to report the Tensor Core mode in the verbose
            // output, we switch
            // to using the new *Get*_v7() call.  Since the function signature
            // of *Get*_v7() matches
            // that of *Find*(), we can unify the find-vs-get logic by using
            // function pointers.

            // Forward Algorithm Find/Get() v7
            std::vector<cudnnConvolutionFwdAlgoPerf_t> fwd_results(
                MaxForwardAlgos(s->dnn_handle_));
            int actual_fwd_algos = 0;
            auto fwd_algo_discoverer =
                param_.cudnn_tune.value() == conv::kOff
                    ? cudnnGetConvolutionForwardAlgorithm_v7
                    : cudnnFindConvolutionForwardAlgorithm;
            CUDNN_CALL((*fwd_algo_discoverer)(
                s->dnn_handle_, in_desc_, filter_desc_, forward_conv_desc_,
                out_desc_, fwd_results.size(), &actual_fwd_algos,
                fwd_results.data()));
            fwd_results.resize(actual_fwd_algos);
            AlgoFinalSelect<cudnnConvolutionFwdAlgoPerf_t,
                            cudnnConvolutionFwdAlgo_t>(
                fwd_results, "forward", workspace_byte, &forward_algo_);
            ...
            CUDNN_CALL((*bwd_filter_algo_discoverer)(
                s->dnn_handle_, in_desc_, out_desc_, back_conv_desc_w_,
                filter_desc_, bwd_filt_results.size(), &actual_bwd_filter_algos,
            ...
            AlgoFinalSelect<cudnnConvolutionBwdFilterAlgoPerf_t,
                ...
                bwd_filt_results, "backprop-to-filter", workspace_byte,
                &back_algo_w_);

            // Backprop-to-Data Algorithm Find/Get() v7
            auto max_bwd_data_algos = MaxBackwardDataAlgos(s->dnn_handle_);
            std::vector<cudnnConvolutionBwdDataAlgoPerf_t> bwd_data_results(
                max_bwd_data_algos);
            ...
            CUDNN_CALL((*bwd_data_algo_discoverer)(
                s->dnn_handle_, filter_desc_, out_desc_, back_conv_desc_,
                in_desc_, bwd_data_results.size(), &actual_bwd_data_algos,
                bwd_data_results.data()));
            bwd_data_results.resize(actual_bwd_data_algos);
            AlgoFinalSelect<cudnnConvolutionBwdDataAlgoPerf_t,
                            cudnnConvolutionBwdDataAlgo_t>(
                bwd_data_results, "backprop-to-data", workspace_byte,
                &back_algo_);
#else
            // CUDNN_MAJOR < 7
            ...
            if (CUDNN_MAJOR == 6 && param_.layout.value() == mshadow::kNHWC) {
              ...
              // supported.  Hard-coded this since the algo find() or get()
              // throws an FPE.
              forward_algo_.Set(CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM,
                                false);
            } else if (!param_.cudnn_tune.value()) {
              cudnnConvolutionFwdAlgo_t fastest_fwd_algo;
              CUDNN_CALL(cudnnGetConvolutionForwardAlgorithm(
                  s->dnn_handle_, in_desc_, filter_desc_, forward_conv_desc_,
                  out_desc_, CUDNN_CONVOLUTION_FWD_SPECIFY_WORKSPACE_LIMIT,
                  workspace_byte, &fastest_fwd_algo));
              ...
              CUDNN_CALL(cudnnFindConvolutionForwardAlgorithm(
                  s->dnn_handle_, in_desc_, filter_desc_, forward_conv_desc_,
                  out_desc_, kMaxAlgos, &nalgo, fwd_algo));
              i = 0;
              while (i < nalgo &&
            ...
            if (!param_.cudnn_tune.value()) {
              ...
              CUDNN_CALL(cudnnGetConvolutionBackwardFilterAlgorithm(
                  ...
                  workspace_byte, &fastest_bwd_filt_algo));
              ...
              CUDNN_CALL(cudnnFindConvolutionBackwardFilterAlgorithm(
                  s->dnn_handle_, in_desc_, out_desc_, back_conv_desc_w_,
                  filter_desc_, kMaxAlgos, &nalgo, bwd_filter_algo));
              i = 0;
              while (i < nalgo &&
                ...
                ++i;
              if (i == nalgo) {
                LOG(FATAL) << "Failed to find a backward filter convolution "
                              "algorithm.";
              } else {
            ...
            if (!param_.cudnn_tune.value()) {
              ...
              CUDNN_CALL(cudnnFindConvolutionBackwardDataAlgorithm(
                  s->dnn_handle_, filter_desc_, out_desc_, back_conv_desc_,
                  in_desc_, kMaxAlgos, &nalgo, bwd_data_algo));
              i = 0;
              while (i < nalgo &&
              ...
              if (i == nalgo) {
                LOG(FATAL)
                    << "Failed to find a backward data convolution algorithm.";
              } else {
            ...
            // We're caching results of *Get* as well as *Find*, but these
            // records
            // will be held distinctly because param_.cudnn_tune is part of the
            // key.
            CuDNNConvAlgoReg::Get()->Register(
                param_, in_shape, out_shape, dtype_, cudnn_forward_compute_type,
                cudnn_backward_compute_type, SMArch(ctx.dev_id),
                this->forward_algo_, this->back_algo_, this->back_algo_w_);
            on_complete();
          },
          ctx, {}, {var});
...
// once again set the mathType in all cases.
#if CUDNN_MAJOR >= 7
    CUDNN_CALL(cudnnSetConvolutionMathType(forward_conv_desc_,
                                           forward_algo_.MathType()));
    CUDNN_CALL(
        cudnnSetConvolutionMathType(back_conv_desc_, back_algo_.MathType()));
    CUDNN_CALL(cudnnSetConvolutionMathType(back_conv_desc_w_,
                                           back_algo_w_.MathType()));
#endif
  }

  // Look over the results from *Find*() or *Get*() and pick the fastest algo
  // given possible
  // workspace constraints.
  template <typename PerfType, typename AlgoType>
  void AlgoFinalSelect(const std::vector<PerfType> &perf_results,
                       std::string kernel_name, size_t workspace_byte,
    ...
    LOG(FATAL) << "Failed to" << mode << "any " << kernel_name
  ...
  void GetTempSize(const OpContext &ctx) {
    ...
    CUDNN_CALL(cudnnGetConvolutionBackwardDataWorkspaceSize(
        s->dnn_handle_, filter_desc_, out_desc_, back_conv_desc_, in_desc_,
        back_algo_.AlgoNumber(), &back_size));
    CUDNN_CALL(cudnnGetConvolutionBackwardFilterWorkspaceSize(
        s->dnn_handle_, in_desc_, out_desc_, back_conv_desc_w_, filter_desc_,
        back_algo_w_.AlgoNumber(), &back_size_w));
    backward_workspace_byte_ = std::max(back_size, back_size_w);
    CUDNN_CALL(cudnnGetConvolutionForwardWorkspaceSize(
        s->dnn_handle_, in_desc_, filter_desc_, forward_conv_desc_, out_desc_,
        forward_algo_.AlgoNumber(), &forward_workspace_byte_));
  ...
  template <int dim> inline Shape<dim> Strides(const TShape &s) {
  ...
  engine::OprExecStat *start_span(const std::string &name, std::string category,
                                  span_props props) {
#if MXNET_USE_PROFILER
