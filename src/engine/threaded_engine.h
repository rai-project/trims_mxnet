...
#define DEFINE_ENGINE_DEBUG_INFO(Type)
  ...
  public:
    ...
    void PushAsync(AsyncFn exec_fun, Context exec_ctx, std::vector<VarHandle> const& const_vars,
                   std::vector<VarHandle> const& mutable_vars, FnProperty prop = FnProperty::kNormal, int priority = 0,
                   const char* opr_name = nullptr) override;
  ...
  private:
     ...
     */
    void CheckDuplicate(std::vector<VarHandle> const& const_vars, std::vector<VarHandle> const& mutable_vars);
    /*!
    ...
    inline void BulkAppend(SyncFn exec_fn, Context exec_ctx, std::vector<VarHandle> const& const_vars,
                           std::vector<VarHandle> const& mutable_vars) {
      BulkStatus& bulk_status = *BulkStatusStore::Get();
      ...
      ++bulk_status.count;
      bulk_status.const_vars.insert(bulk_status.const_vars.end(), const_vars.begin(), const_vars.end());
      bulk_status.mutable_vars.insert(bulk_status.mutable_vars.end(), mutable_vars.begin(), mutable_vars.end());
    ...
    inline void BulkFlush() {
      ...
      this->PushAsync(
          [fn](RunContext ctx, CallbackOnComplete on_complete) {
            fn(ctx);
            on_complete();
          },
          bulk_status.ctx, bulk_status.const_vars, bulk_status.mutable_vars, FnProperty::kNormal, 0, "ImperativeBulk");
