...
inline int MXAPIGetFunctionRegInfo(const FunRegType *e, const char **name,
  ...
  *arg_descriptions =
      dmlc::BeginPtr(ret->ret_vec_charp) + (e->arguments.size() * 2);
  API_END();
}
...
int MXSetProfilerConfig(int mode, const char *filename) {
  ...
  engine::Profiler::Get()->SetConfig(engine::Profiler::ProfilerMode(mode),
                                     std::string(filename));
#else
...
int MXNDArrayCreateSparseEx(int storage_type, const mx_uint *shape,
  ...
  *out = new NDArray(
      NDArrayStorageType(storage_type), TShape(shape, shape + ndim),
      Context::Create(static_cast<Context::DeviceType>(dev_type), dev_id),
      delay_alloc != 0, dtype, aux_types, aux_shapes);
  API_END();
}
...
int MXNDArrayLoad(const char *fname, mx_uint *out_size, NDArrayHandle **out_arr,
...
int MXNDArraySlice(NDArrayHandle handle, mx_uint slice_begin, mx_uint slice_end,
                   NDArrayHandle *out) {
  NDArray *ptr = new NDArray();
  API_BEGIN();
  *ptr =
      static_cast<NDArray *>(handle)->SliceWithRecord(slice_begin, slice_end);
  *out = ptr;
  API_END_HANDLE_ERROR(delete ptr);
}
...
MXNET_DLL int MXNDArrayReshape(NDArrayHandle handle, int ndim, int *dims,
  ...
  for (int i = 0; i < ndim; ++i) {
    int dim = dims[i];
    if (dim == -1) {
      CHECK_EQ(pos, -1) << "Invalid new shape " << new_shape
...
int MXFuncGetInfo(FunctionHandle fun, const char **name,
  ...
  return MXAPIGetFunctionRegInfo(static_cast<const NDArrayFunctionReg *>(fun),
                                 name, description, num_args, arg_names,
                                 arg_type_infos, arg_descriptions, return_type);
}
...
int MXDataIterGetIterInfo(DataIterCreator creator, const char **name,
  ...
  return MXAPIGetFunctionRegInfo(e, name, description, num_args, arg_names,
                                 arg_type_infos, arg_descriptions, NULL);
}
...
void MXKVStoreSetUpdaterImpl(KVStoreHandle handle, MXKVStoreUpdater updater,
                             void *updater_handle) {
  ...
  std::function<void(int, const NDArray &, NDArray *)> updt =
      [updater_temp, updater_handle_temp](int key, const NDArray &recv,
                                          NDArray *local) {
...
int MXKVStoreSetUpdaterEx(KVStoreHandle handle, MXKVStoreUpdater updater,
  ...
  std::function<void(const std::string &, const NDArray &, NDArray *)> updt =
      [updater_temp, updater_handle_temp](const std::string &key,
                                          const NDArray &recv, NDArray *local) {
        NDArray *recv_copy = new NDArray();
        *recv_copy = recv;
        NDArray *local_copy = new NDArray();
        *local_copy = *local;
...
int MXKVStoreRunServer(KVStoreHandle handle,
  ...
  auto ctrl = [controller_temp,
               controller_handle_temp](int head, const std::string &body) {
...
int MXKVStoreSendCommmandToServers(KVStoreHandle handle, int cmd_id,
                                   const char *cmd_body) {
  API_BEGIN();
  static_cast<KVStore *>(handle)->SendCommandToServers(cmd_id,
                                                       std::string(cmd_body));
  API_END();
}
...
int MXKVStoreGetNumDeadNode(KVStoreHandle handle, const int node_id,
  ...
  API_BEGIN();
  *number =
      static_cast<KVStore *>(handle)->get_num_dead_node(node_id, timeout_sec);
  API_END();
}
...
#else
  LOG(FATAL) << "Compile with USE_CUDA=1 and ENABLE_CUDA_RTC=1 to have CUDA "
                "runtime compilation.";
#endif
  API_END();
}
