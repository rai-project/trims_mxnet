...
void SetNDInputsOutputs(const nnvm::Op *op, std::vector<NDArray *> *ndinputs,
                        std::vector<NDArray *> *ndoutputs, int num_inputs,
                        const NDArrayHandle *inputs, int *num_outputs,
                        int infered_num_outputs, int num_visible_outputs,
  ...
  if (out_array == nullptr) {
    ...
    *num_outputs = num_visible_outputs;
    ...
    CHECK(*num_outputs == infered_num_outputs ||
          *num_outputs == num_visible_outputs)
...
void MXImperativeInvokeImpl(AtomicSymbolCreator creator, int num_inputs,
  ...
  imperative::SetNumOutputs(op, attrs, num_inputs, &infered_num_outputs,
                            &num_visible_outputs);

  std::vector<NDArray *> ndinputs, ndoutputs;
  SetNDInputsOutputs(op, &ndinputs, &ndoutputs, num_inputs, inputs, num_outputs,
                     infered_num_outputs, num_visible_outputs, outputs);

  auto state =
      Imperative::Get()->Invoke(Context::CPU(), attrs, ndinputs, ndoutputs);
  ...
  if (*outputs == nullptr) {
    ...
    *outputs =
        reinterpret_cast<NDArrayHandle *>(dmlc::BeginPtr(ret->ret_handles));
  }
}
...
int MXCreateCachedOp(SymbolHandle handle, CachedOpHandle *out) {
  ...
  API_BEGIN();
  *out = new std::shared_ptr<Imperative::CachedOp>(new Imperative::CachedOp(
...
int MXAutogradBackward(mx_uint num_output, NDArrayHandle *output_handles,
  ...
  return MXAutogradBackwardEx(num_output, output_handles, ograd_handles, 0,
                              nullptr, retain_graph, false, true, nullptr,
                              nullptr);
}
