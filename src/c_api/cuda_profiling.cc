...
namespace detail {
static std::mutex profiler_mutex; // To prevent multiple threads from accessing
                                  // the profiler simultaneously
}
...
range::handle_t range_start(const std::string &description,
                            ::cuda::profiling::range::Type type,
                            color_t color) {
  std::lock_guard<std::mutex>{detail::profiler_mutex};
  ...
  static_assert(sizeof(range::handle_t) == sizeof(nvtxRangeId_t),
                "Can't use range::handle_t as a cover for nvtxRangeId_t - "
...
scoped_range_marker::scoped_range_marker(const std::string &description,
                                         profiling::range::Type type) {
...
void start() { CUDA_CHECK_CALL(cudaProfilerStart(), "Starting to profile"); }

void stop() { CUDA_CHECK_CALL(cudaProfilerStop(), "Starting to profile"); }
