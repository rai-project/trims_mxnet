...
int MXExecutorBind(SymbolHandle symbol_handle, int dev_type, int dev_id,
                   ...
                   ExecutorHandle *out) {
  return MXExecutorBindX(symbol_handle, dev_type, dev_id, 0, nullptr, nullptr,
...
int MXExecutorBindEX(SymbolHandle symbol_handle, int dev_type, int dev_id,
  ...
  *out = Executor::Bind(*symb, ctx, ctx_map, in_args_vec, arg_grad_vec,
                        grad_req_vec, aux_states_vec,
 ...
 * _bind_ith_exec
 * \param updated_shared_buffer_name_list updated shared data array names after
 * binding
...
int MXExecutorSimpleBind(
    ...
    const mx_uint num_g2c_keys, const char **g2c_keys, const int *g2c_dev_types,
    const int *g2c_dev_ids, const mx_uint provided_grad_req_list_len,
    const char **provided_grad_req_names, const char **provided_grad_req_types,
    ...
    NDArrayHandle **in_args, NDArrayHandle **arg_grads, mx_uint *num_aux_states,
  ...
  if (0 == provided_grad_req_list_len && nullptr == provided_grad_req_names &&
      nullptr != provided_grad_req_types) { // string, grad_req='write'
    CHECK_EQ(req_map.count(provided_grad_req_types[0]), 1U)
        << "grad_req=" << provided_grad_req_types[0]
        << " is not a valid input in simple_bind; "
    ...
    grad_req_type = "string";
             ...
             nullptr !=
                 provided_grad_req_types) { // list, grad_req=['null', 'write']
    grad_req_type = "list";
    CHECK_EQ(provided_grad_req_list_len, in_arg_names.size())
        << "The length of grad_req list does not match the number of input "
           "arguments in simple_bind, "
           "expected "
        << in_arg_names.size() << ", provided " << provided_grad_req_list_len;
  } else if (provided_grad_req_list_len > 0 &&
             nullptr != provided_grad_req_names &&
             nullptr != provided_grad_req_types) { // dict, grad_req=['lhs':
                                                   // 'null', 'rhs': 'write']
    grad_req_type = "dict";
    ...
    for (mx_uint i = 0; i < provided_grad_req_list_len; ++i) {
      CHECK_EQ(req_map.count(provided_grad_req_types[i]), 1U)
          << "grad_req=" << provided_grad_req_types[i]
          << " is not a valid input in simple_bind; "
             "only \'null\', \'write\', and \'add\' are supported";
      provided_grad_req_map[provided_grad_req_names[i]] =
          provided_grad_req_types[i];
    }
  ...
  for (mx_uint i = 0; i < num_provided_arg_shapes; ++i) {
    auto p = arg_shape_map.emplace(
        provided_arg_shape_names[i],
  ...
  *out = Executor::SimpleBind(
      *sym, ctx, ctx_map, in_arg_ctx_vec, arg_grad_ctx_vec, aux_state_ctx_vec,
      arg_shape_map, arg_dtype_map, arg_stype_map, grad_req_type_vec,
  ...
  ret->ret_handles.reserve(in_arg_vec.size() + arg_grad_vec.size() +
                           aux_state_vec.size() + shared_buffer_map.size());
  size_t nd_idx = 0;
