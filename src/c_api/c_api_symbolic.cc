...
int MXSymbolGetAtomicSymbolInfo(AtomicSymbolCreator creator, const char **name,
  ...
  return NNGetOpInfo(creator, name, description, num_args, arg_names,
  ...
  for (nn_uint i = 0; i < num_param; ++i) {
    ...
    for (const auto &k : kHiddenKeys) {
      ...
      if (pos == 0) {
        ...
        os << "setting variable attributes with " << keys[i]
           << " is deprecated. "
           << "please instead use\nw = Variable(" << k << "=" << vals[i]
           << ")\n"
...
int MXSymbolFree(SymbolHandle symbol) { return NNSymbolFree(symbol); }
...
int MXSymbolGetAttr(SymbolHandle symbol, const char *key, const char **out,
  ...
  if (s->GetAttr(key, &(ret->ret_str))) {
    ...
    *out = nullptr;
    *success = 0;
    if (std::find(kHiddenKeys.begin(), kHiddenKeys.end(), key) !=
        kHiddenKeys.end()) {
...
int MXSymbolListAttr(SymbolHandle symbol, mx_uint *out_size,
  ...
  for (const auto &tp : attr) {
    attr_list.emplace_back(std::get<0>(tp) + kNamespaceSeparator +
                           std::get<1>(tp));
    attr_list.emplace_back(std::get<2>(tp));
    if (find(kReplacedHiddenKeys.begin(), kReplacedHiddenKeys.end(),
             std::get<1>(tp)) != kReplacedHiddenKeys.end()) {
      attr_list.push_back(
          std::get<0>(tp) + kNamespaceSeparator +
...
int MXSymbolListAttrShallow(SymbolHandle symbol, mx_uint *out_size,
  ...
  for (const auto &kv : attr) {
    ...
    if (find(kReplacedHiddenKeys.begin(), kReplacedHiddenKeys.end(),
             kv.first) != kReplacedHiddenKeys.end()) {
...
int MXSymbolCreateFromFile(const char *fname, SymbolHandle *out) {
  ...
  nnvm::Graph g;
  g.attrs["json"] = std::make_shared<nnvm::any>(std::string(
...
void MatchArguments(
    ...
    std::vector<AttrType> *arg_attrs, const char *source) {
  auto &arg_nodes = idx.input_nodes();
  ...
  if (nmatched != known_arg_attrs.size()) {
    ...
    for (const auto &kv : known_arg_attrs) {
      ...
      if (keys.count(key) == 0) {
        LOG(FATAL) << source << "Keyword argument name " << key << " not found."
                   << msg.str();
      }
    }
  }
...
int MXSymbolInferShape(SymbolHandle sym, mx_uint num_args, const char **keys,
  ...
  nnvm::ShapeVector arg_shapes(g.indexed_graph().input_nodes().size(),
                               TShape());
  ...
  // copy data back
  MXAPIThreadLocalEntry::SetupShapeArrayReturnWithBuffer(
