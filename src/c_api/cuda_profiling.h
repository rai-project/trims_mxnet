...
struct color_t {
  ...
  underlying_type as_hex() {
    return ((underlying_type)alpha) << 24 | ((underlying_type)red) << 16 |
           ((underlying_type)green) << 8 | ((underlying_type)blue) << 0;
  }
...
range::handle_t range_start(const std::string &description, range::Type type,
                            color_t color);

inline range::handle_t range_start(const std::string &description,
                                   range::Type type) {
...
/**
 * A RAII class whose scope of existence is reflected as a range in the
 * profiler. Use it in the scope in which you perform some interesting
 * operation, e.g. perform a synchronous I/O operation (and have it conclude of
 * course), or launch and synch several related kernels.
 */
