...
public:
  ResourceManagerImpl() noexcept(false) : global_seed_(0) {
    ...
    cpu_space_.reset(
        new ResourceTempSpace(Context::CPU(), cpu_temp_space_copy_));
...
private:
  ...
  template <typename xpu> struct ResourceRandom {
  ...
  template <typename xpu> struct ResourceParallelRandom {
    /*! \brief the context of the PRNG */
    Context ctx;
    ...
    std::atomic<size_t> curr_ptr;
    /*! \brief constructor */
    explicit ResourceParallelRandom(Context ctx, size_t ncopy,
    ...
    ~ResourceParallelRandom() {
      for (size_t i = 0; i < sampler.size(); ++i) {
        ...
        Engine::Get()->DeleteVariable(
            [r](RunContext rctx) {
              MSHADOW_CATCH_ERROR(
                  common::random::RandGenerator<xpu>::FreeState(r));
