...
namespace resource {
  ...
  public:
    ResourceManagerImpl() noexcept(false) : global_seed_(0) {
      ...
      cpu_rand_.reset(new ResourceRandom<cpu>(Context::CPU(), global_seed_));
      cpu_space_.reset(new ResourceTempSpace(Context::CPU(), cpu_temp_space_copy_));
      cpu_parallel_rand_.reset(new ResourceParallelRandom<cpu>(Context::CPU(), cpu_native_rand_copy_, global_seed_));
    }
    ~ResourceManagerImpl() {
        ...
        switch (req.type) {
          ...
          case ResourceRequest::kParallelRandom: {
            ...
            return gpu_parallel_rand_
                .Get(
                    ctx.dev_id,
                    [ctx, this]() { return new ResourceParallelRandom<gpu>(ctx, gpu_native_rand_copy_, global_seed_); })
                ->GetNext();
          }
          default:
            LOG(FATAL) << "Unknown supported type " << req.type;
...
#if MXNET_USE_CUDA
      gpu_rand_.ForEach([seed](size_t i, ResourceRandom<gpu> *p) { p->Seed(seed); });
      gpu_parallel_rand_.ForEach([seed](size_t i, ResourceParallelRandom<gpu> *p) { p->Seed(seed); });
#endif
    }
  ...
  private:
    ...
    struct ResourceRandom {
      ...
      ~ResourceRandom() {
        ...
        Engine::Get()->DeleteVariable([r](RunContext rctx) { MSHADOW_CATCH_ERROR(delete r); }, ctx, resource.var);
      }
      // set seed to a PRNG
      inline void Seed(uint32_t global_seed) {
        ...
        Engine::Get()->PushAsync(
            ...
            ctx, {}, {resource.var}, FnProperty::kNormal, 0, PROFILER_MESSAGE("ResourceRandomSetSeed"));
      }
    };
    ...
    struct ResourceTempSpace {
      ...
      /*! \brief constructor */
      explicit ResourceTempSpace(Context ctx, size_t ncopy) : ctx(ctx), space(ncopy), resource(ncopy), curr_ptr(0) {
      ...
      inline void Seed(uint32_t global_seed) {
        for (size_t i = 0; i < sampler.size(); ++i) {
          ...
          Engine::Get()->PushAsync(
              ...
              ctx, {}, {resource[i].var}, FnProperty::kNormal, 0, PROFILER_MESSAGE("ResourceNativeRandomSetSeed"));
        }
